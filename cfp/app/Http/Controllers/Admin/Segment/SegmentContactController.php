...
class SegmentContactController extends Controller
{
    ...
    public function __construct(SegmentContactRepository $repository,
                                SegmentContactValidator $validator)
    {
    ...
    public function index(AdminRequest $request, $id)
    {
        ...
        $dados = $this->repository->orderBy('created_at', 'desc')->scopeQuery(function ($query) use ($id) {
            return $query->where('segment_id', $id);
        })->paginate(20);

        return view('admin.segment.contact.index', compact('dados', 'config', 'id'));
    }

    public function header()
    {
        $config['title'] = "Segmentos Contatos";
    ...
    public function show($id)
    {
        ...
        return view('admin.segment.contact.show', compact('dados', 'config'));
    }
    ...
    public function export(Request $request, $id)
    {
        ...
        header("Content-Disposition: attachment; filename=\"Segmentos_Contatos_" . date('Ymds') . ".csv\"");

        $listar = $this->repository->findByField('segment_id', $id);
        ...
        foreach ($listar as $dados) {
            ...
            $item = '"';
            $item .= utf8_decode($dados->segment->name) . '";"';
